...
const Page = () => {
  ...
  // Auto-join collaboration when arriving via invite link.
  // Invite links generated by the UI include `?collab=1`.
  React.useEffect(() => {
    if (!id) return;
    const autoJoin = searchParams?.get("collab") === "1";
    if (autoJoin && !collab.isCollaborationActive) {
